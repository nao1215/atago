...
- `http:` steps keep up to 256 idle connections per host instead of Go's default of two. Scenarios run concurrently and usually call one API, so most requests closed their connection and the next one dialed again: 2000 plain-HTTP requests from 500 concurrent scenarios opened about 950 connections, and now open about as many as run at once. A server that speaks HTTP/1.1 over TLS no longer costs a handshake per request; HTTP/2 already multiplexed.
- atago collects garbage less often. Its live heap stays at a few megabytes while it allocates steadily, and the default collector setting spent about a third of its CPU time collecting; the target is now 400% (GOGC=400) with a 2GiB soft memory limit, which cuts atago's own CPU time by about 30% on sqly's suite and on a spec of 1000 scenarios, for about 20MB more peak memory. Setting GOGC or GOMEMLIMIT in the environment still takes precedence.
- `--parallel` defaults to four scenarios per CPU instead of one. A scenario spends most of its time waiting on the program it started, on a pipe, a prompt or a port, so one per CPU left CPUs idle. Limited to four CPUs, sqly's suite of 1028 scenarios runs in 3.2s instead of 5.9s and atago's own suite in 4.7s instead of 10.3s; on two CPUs sqly's suite takes 5.8s at six instead of 11.3s at two. A suite that only runs short commands takes the same time at either setting. `--parallel N` still sets it, and `--parallel 1` still serializes.
- A pty `expect` sees output the moment it arrives. It re-read the transcript every 10ms, so each expect waited up to 10ms after the program had already answered; it is now woken by the output itself, with at most one scan per millisecond. sqly's interactive suite of 41 scenarios, which waits on its prompt dozens of times per scenario, runs in 0.66s instead of 1.45s. `expect_screen` still renders on its 10ms poll, since each render replays the whole transcript. A session now reaches its next `send` sooner, so a spec that sends a key the moment some text appears, or quits right after the key that starts background work, can hit a program that is not ready yet; wait for what the program shows with `expect_screen` and `stable_for`, as atago's own lazygit suite now does.
- atago reads YAML with a reader of its own, and github.com/goccy/go-yaml is no longer a dependency. Specs, directory manifests and the YAML a `yaml:` matcher inspects all go through it. It parses about four times faster than the previous reader and allocates a sixth of the memory, so on sqly's suite of 1028 scenarios atago spends about 43% less CPU time, allocates 54% less and peaks at about 49MB instead of 60MB, and a spec of 1000 scenarios runs in 146ms instead of 193ms. The binary is 0.6MB smaller. Every YAML file in this repository and twelve other nao1215 repositories, 5301 in all, loads to the same result as before.
- A value written in the wrong shape names its path: `scenarios[0].steps[1].assert.stdout.matches: expected a string, not a list`, where the previous reader named a Go type and the outermost struct field. The hints under it are unchanged.
- The first problem in the file is the one reported. When a mapping had several unknown keys, which one the error named changed from run to run, and a later unknown key could be reported before an earlier value of the wrong shape.
- `atago manifest` gives the position of a step and of a runner as the start of its mapping, the line and column of its first key, instead of the position of the first value inside it.
- A `yaml:` matcher reads `+.inf` as infinity, keeps the trailing blanks of the last line of a `|-` block and the blanks an escape writes before a line break in a double-quoted string, and accepts an anchor or a tag on a mapping key, as YAML specifies.
...
- `assert: mock:` takes `query:`, matching one query-string parameter of the last matching request by name, with the same `equals`, `contains` and `matches` as `header:`. Routes match on the path alone, so what a client asked for, such as the search term or the page cursor, had no assertion at all, and the loader's own advice for a route declared with a query string (assert the query with the mock target's matchers) pointed at a matcher that did not exist. The value is compared decoded, and a missing parameter reads as the empty string.
- `pty.graphics: kitty` makes the pty answer like a terminal that draws images with the kitty graphics protocol, and `screen.images` asserts the images on screen. A program that shows pictures inline asks the terminal first, and one that requires them exits when the answer is no, so on the plain pty it could only ever be tested failing. With the key set, the capability query is acknowledged, the cell-size query (`CSI 16 t`) reports 10x20-pixel cells, and every directly transmitted image (PNG, raw RGB or RGBA, chunked or zlib-compressed) is decoded and kept until the program deletes it (#684): a delete command (`a=d`) takes the images it names off the screen, by id, number, id range, or all of them, and an image hidden with a lower-case target is back after `a=p`, so a step that turns pictures off can assert `images: { count: 0 }` instead of starting the program a second time. Targets that name a screen position, a z-index, or animation frames cannot be followed from the transcript and leave the set as it is. `screen.images` takes `count` or `min_count`, and `contains` entries that borrow the `image:` assertion's constraints: size bounds, `alpha`, and `similar_to` with `max_diff`. An entry holds when some image on screen meets all of its constraints, since a program that downloads images concurrently draws them in no fixed order. `expect_screen:` takes the same `images:` to wait for an image to arrive, except `similar_to`, whose baseline needs the spec directory a mid-session wait does not have. On a failed `contains`, every image on screen is written to `--artifacts-dir`. On Windows the key works too (#676): the console host built into Windows re-renders a program's output and drops the graphics protocol, both the query and the images, so a step with the key runs behind the Windows Terminal console host instead (OpenConsole, from Microsoft's MIT-licensed ConPTY package), which atago embeds for amd64 and arm64 and writes to the user cache on first use. That host still drops an APC string written to its input, so the graphics answer is sent to it as key presses, which it hands to the program unchanged. Everything a session types goes to that host as key presses too (#678). Once it has received one, it takes every raw ESC for the start of an escape sequence, so after a program had probed the terminal, `{key: esc}` never arrived and held back the key after it. Text is sent as one key press per UTF-16 code unit (a character outside the BMP as its surrogate pair), a named key with its virtual-key code and modifiers, and any other escape sequence, such as the bracketed-paste markers or a mouse report, unchanged for the host to parse. Each key goes down and comes back up, as it does from Windows Terminal, because the host folds identical key-down records that follow each other into one.

### Changed

- google.golang.org/grpc is back at 1.83.2. A dependency update after v0.23.0 moved it to 1.84.0, the version GO-2026-6443 (GHSA-2v4p-qf9q-27wj) names, although no 1.84 release carries the fix. atago only dials gRPC as a client, so the advisory does not reach it, but the release binary should not ship a version the advisory names. Dependabot now skips 1.84.0 and will offer the next release.

### Fixed

- A selection `--ci` refused now reads as a failure in every report. `atago run --ci --tag typo` exited 3 with ATG3203 on stderr but printed `PASSED  0 scenarios`, and the JSON, JUnit, TAP and GHA reports described a clean run of nothing. The console and GHA summaries now read FAILED and end in "the selection matched nothing", JSON carries `empty_selection` with the diagnostic, JUnit an errored `selection` testcase, and TAP a failing `selection` point.
- `atago snapshot update --help` printed the usage of `atago run`, including `--update-snapshots`, which the command already implies. It now names itself and leaves that flag out. The `--profile` flag's placeholder reads `NAME` instead of `profile` in the help of both commands.
- An `http:` step could fail with "CloseIdleConnections called" when another scenario ran at the same time. HTTP steps used Go's shared default transport, whose idle connections anything in the process may close, and a request about to reuse one of them failed although nothing was wrong with the scenario. HTTP steps now share a connection pool of their own.
- The documentation site printed `--help`, `--force` and every other double-hyphen flag outside a code span as an en dash (`–help`), so a flag copied from a page did not work and 1069 heading links on the real-world pages led nowhere. Typographic substitution is now off. The spec-key reference had no section for seven schema types (`forall`, `generator`, `deterministic`, `screen`, and three matcher shapes), so none of the `screen:` keys appeared and the rows linking to them were dead; a type the section list does not name is now appended by itself. A merge had left a code block in the cookbook unclosed, which turned every recipe from "Track a known bug" on into code, on GitHub as well; a test now refuses an unclosed block in any Markdown document. The pages also disagreed with the code in smaller ways: the `record` summary and run footers of the examples, a `--profile` example whose build lost its `cwd` (the profile replaces the whole `build` block), a GitLab example that needs the image's entrypoint cleared, `ATAGO_SHELL` said to require an absolute path, the comparison page naming v0.22.0 and vt10x, the pty schema description still saying POSIX-only, and a release-note link to a README section that lives on the install page.
...
- `permutation_of:` and `subset_of:` are stream matchers that compare the LINES of a stream against a reference as multisets rather than its bytes against a text — the relation half of [metamon](https://github.com/nao1215/metamon), whose generators arrived as `forall:` in the entry below. `equals` was the only relation a spec could state, and it is the wrong one wherever the order is not part of the contract: a listing that comes back in map order, a run with `--parallel` against a serial one, `ls` against `ls -r`. `permutation_of: "${serial}"` says "the same lines, in any order", and `subset_of: "${all}"` says "every line was already in the reference", which is what a filtering flag satisfies. Both are usable with a literal, so output whose order is genuinely unspecified can be pinned exactly instead of with three `contains` matchers that would also pass on a fourth, unexpected line. Lines are compared as multisets, so a line printed twice needs two in the reference — printing something twice is the bug a set comparison hides. CRLF is folded as it is for every stream text matcher, one trailing newline is not a line, and nothing else is normalized. A failure names the difference in both directions: the lines missing from the output and the lines it produced that the reference does not have.
- `forall:` generates a scenario's inputs instead of listing them, expanding one template into one instance per generated row exactly as `matrix:` does — property-based testing, ported from the Gleam library [metamon](https://github.com/nao1215/metamon), in a spec file. The opt-in is one line: `forall: {vars: {text: ascii}}` binds `${text}` to generated printable text and runs the scenario ten times. The kinds are `int`, `bool`, `digits`, `alpha`, `alphanumeric`, `ascii` and `unicode`, with `min`/`max` narrowing an integer range or a string length, and `one_of` for a fixed set of choices. What keeps this a spec rather than a lottery is that nothing reads the clock: the values are drawn from a seeded generator the spec itself carries, so the same file tests the same inputs on every machine and in every run, `atago list` and `atago doc` show them before anything runs, and a failing instance carries its input in its name — `parses [text=a b|c]` — which is the reproduction. Asking for different inputs is an edit (`seed:`), not a re-run. Boundary values come first, the way metamon consumes a generator's edges: the empty string and the longest one, the ends of an integer range, both booleans, every choice. Rows are deduplicated, so `runs:` is an upper bound and two choices expand to two scenarios however large it is. Generated `ascii` never carries a quote, a backslash, a backtick or a `$`, because a value that could change how a command line is read would report a tokenizer error instead of the program's behavior; arbitrary text belongs on `stdin:`, in `env:`, or in a fixture, which is what the example and the cookbook recipe show.
...
- Dependencies updated: golang.org/x/crypto 0.57.0, golang.org/x/image 0.46.0, golang.org/x/sys 0.48.0, golang.org/x/term 0.46.0, modernc.org/sqlite 1.59.0, go-sql-driver/mysql 1.10.1, mattn/go-runewidth 0.0.30, ohler55/ojg 1.28.6, bmatcuk/doublestar 4.10.2, modernc.org/libc 1.77.0, chromedp/cdproto, charmbracelet/ultraviolet and charmbracelet/x/vt. The go directive reads 1.26.0 rather than 1.26 because the x/ family now declares that form and the toolchain refuses the shorter one against it; the floor is the same release it always was.
- google.golang.org/grpc stays at 1.83.2. 1.84.0 is inside GO-2026-6443 (GHSA-2v4p-qf9q-27wj), a panic in gRPC servers using xDS, and no release carries the fix yet. atago only dials gRPC as a client, so the advisory does not reach it, but the release binary should not ship a version the advisory names.
...
- `file.equals` and `file.equals_file` expand `${name}` like every other matcher argument. They were the only two the engine skipped, so `equals_file: "${fixtures}/expected.json"` looked for a file whose name contained those eleven literal characters — in the one place a directory manifest's `fixtures_dir` exists to be named from — and `equals: "${captured}"` could never match anything a program wrote. `changes.ignore` had the same gap next to the three categories beside it, which do expand, so a path a `store` had just captured could not be named there either. Because the walker that expands is also the one `atago explain` and `atago manifest` collect variables with, both were under-reporting the variables those scenarios use; that is fixed by the same change. A `snapshot:` name stays literal on purpose, and so do the header and mock request identifiers and the duration bounds the loader parses before any variable exists — each now recorded, with its reason, in a test that fills every text field an assert can carry and fails when one of them is neither expanded nor accounted for.
...
- Third-party E2E suite for [curl](https://curl.se/), in `test/e2e/thirdparty/curl/`, and the first one whose subject is what a client sends rather than what it prints. Every scenario declares an atago mock server, runs curl against it, and asserts the recorded request: `-d` making a form POST and joining its parts, `-d @file` stripping the newlines `--data-binary` keeps, `--json` setting both Content-Type and Accept and sending the document whole, `-F` building a multipart body whose boundary is announced in the header, `-H` replacing a header curl adds on its own and an empty value deleting one, `-u` encoding Basic credentials, `-X GET` renaming the method while still sending the body, `-G` moving the same data into the query string and leaving none, and `-L` turning a redirected POST into a GET with no body — visible only because the server recorded both requests. Alongside them, a 404 that is a successful transfer until `--fail` makes it exit 22, a slow route answered with curl's own timeout code 28, and a canned response read three times under `deterministic:`. This is the first use of `mock_servers:` against a real program: until now atago's mock server was exercised only by its own self-hosted specs.
- Third-party E2E suite for [pyenv](https://github.com/pyenv/pyenv), in `test/e2e/thirdparty/pyenv/`, covering only what pyenv does that its rbenv ancestor does not, so the two suites do not repeat each other. A version file can name several versions at once: both are reported as current with the file that chose them, the first decides which `python` runs, and the second still provides a command the first does not have — asserted by running that command through the shim, which is the point of naming several. Alongside it, `whence` answering which installed version has a command and exiting 1 with nothing printed when none does, aliases listed as versions and excluded by `--skip-aliases` while still resolving when selected, and a multi-version file with one uninstalled entry failing with the file named. No Python is installed for any of it: a version is a directory the scenario writes.
- Third-party E2E suite for [asdf](https://asdf-vm.com/), in `test/e2e/thirdparty/asdf/`, which tests a plugin-driven version manager without downloading anything: each scenario writes its own plugin — `bin/list-all`, `bin/download`, `bin/install` — commits it in the workdir, and adds it from that path, so the plugin protocol itself is what is under test. Pinned are which script asdf calls and what it hands over in the environment, the program the plugin actually laid down, the download directory cleaned up after a successful install and kept after a failed one, a plugin refusing a version being reported rather than second-guessed, and `plugin remove` taking the installed versions with it. The resolution half asserts `global` writing the file in the home directory and `local` writing the directory's own, `current` naming which file answered, the shim dispatching by directory while PATH never changes, `ASDF_MYTOOL_VERSION` outranking the file, `which`/`where`/`exec` agreeing on one answer, and the shim's exit-126 refusal quoting the exact install command and the exact file that asked for a version that is no longer there.
- Third-party E2E suite for [mommy](https://github.com/FWDekker/mommy), in `test/e2e/thirdparty/mommy/`, which pins the two obligations a command wrapper has before anything else: the exit status comes back untouched — 0, 1, and an arbitrary 42 — and the message stays on stderr so the wrapped command owns stdout, with `-1` moving it deliberately and `-e "echo … | wc -l"` proving a real pipeline's output passes through clean. Each scenario writes a config file that fixes the messages, so the assertions are about which list was chosen rather than which sentence was drawn from it; templates, `-s` judging a status without running anything, and `-p` making a pipeline's head failure the verdict are covered alongside. Three behaviors are pinned as observed: the config variables cannot be set through the environment, the toggle notice arrives on stdout rather than stderr like every other message, and while mommy is toggled off the wrapped command's failure is swallowed with her — `mommy false` exits 0, so a CI step wrapped in mommy reports success for a build that failed.
- Third-party E2E suite for [nb](https://xwmx.github.io/nb/), in `test/e2e/thirdparty/nb/`, which tests a note-taking CLI by reading the git repositories it keeps its notes in: add, edit and delete are asserted as commits on top of the notebook's own Initialize, beside the files they left. The first run is pinned in both shapes — refused with an explanation when git has no identity to attribute edits to, and completed in a pseudo-terminal where nb asks for a name and email and writes them into the sandboxed home — along with the surprise that the first command initializes the notebook instead of doing what it was asked, and reports success. Notebooks are covered as separate repositories with scoped identifiers, `use` writing the choice to a file that survives between commands, and the refusals, where a missing identifier answers on stderr while an unknown subcommand answers with the same sentence on stdout. The listing is uncolored through a pipe and colored on a terminal, with line-wrap control sequences emitted either way — which is what anything parsing that output has to expect.
- Third-party E2E suite for [transcrypt](https://github.com/elasticdog/transcrypt), in `test/e2e/thirdparty/transcrypt/`, which pins transparent file encryption as what it is — a difference between two views of one file. The working tree holds the plaintext while `git show HEAD:<file>` holds an OpenSSL envelope with no trace of it, and a file the pattern does not name is stored verbatim. The properties that make the scheme livable are asserted alongside it: re-saving identical content leaves `git status` clean, a real change produces a plaintext diff that `--stat` still reports as binary, and two paths holding identical content do not encrypt to the same bytes. The workflow half clones the repository for real inside the scenario workdir — the copy carries ciphertext until it is given the password, and the wrong password is caught and named rather than left as garbage in the tree — and drives `--uninstall` in a pseudo-terminal for both answers: accepted, it removes the configuration and the filter attribute while leaving the plaintext readable and the already-encrypted history untouched; declined, it changes nothing. Every password is invented for the scenario that uses it.
- Third-party E2E suite for [git-extras](https://github.com/tj/git-extras), in `test/e2e/thirdparty/git-extras/`, which asserts each subcommand on the repository afterwards rather than on what it printed. The reporting half pins `summary`, `count`, `effort` and `authors` against a repository the scenario builds commit by commit, and surfaces something a pipeline hides: the author tables of `summary` and `count --all` come from `git shortlog`, which reads standard input when it is not attached to a terminal, so through a pipe the headings arrive with nothing under them. The same commands run in a pseudo-terminal, where the tables are there — the difference asserted rather than described. The writing half pins the branch lifecycle (create, rename, delete, and the merged-branch sweep that keeps unmerged work), `undo` taking back a commit while leaving its content staged, `ignore` appending to the file git actually reads, `alias` writing an alias git itself then runs, and `changelog` collecting the commits after the last tag — and prepending a second copy of that section when run again. `delete-branch` printing an error about a remote-tracking branch that never existed, while exiting 0, is pinned as observed.
- Third-party E2E suite for [git-open](https://github.com/paulirish/git-open), in `test/e2e/thirdparty/git-open/`, which pins the translation from a git remote to a web address as a matrix: ssh, https, the git protocol, an ssh port, a missing `.git` suffix, a GitLab subgroup, a gist, and Bitbucket — whose branch pages live under `/src` rather than `/tree`. Around it are the rules that decide which page: the default branch opening the repository itself while any other opens the branch page, a slash in a branch name kept as a path while a `#` is percent-escaped, a detached head opening the commit it stands on, the `--commit` and `--file` pages with the refusal for a file the repository does not track, and a remote and branch named as arguments. One scenario names a script as `BROWSER` and proves from the log that script writes that the address is handed over as a single argument rather than as a shell string. Two behaviors are pinned as observed: `--issue` on a branch with no number in its name answers with the repository page, and a repository with no remote configured produces `https://origin/origin` at exit 0. No network is reached: every repository is created in the scenario's workdir with a remote that is never contacted.
- Third-party E2E suite for [shdotenv](https://github.com/ko1nksm/shdotenv), in `test/e2e/thirdparty/shdotenv/`, built around the promise that loading a .env file cannot run anything. That claim is asserted the way a security claim should be: three files that try to start a command are each refused, and the proof is that the files they would have created do not exist, with a value carrying `; touch …` accepted as text and still inert while a real command runs with it in the environment. Around it are the strict defaults that make the tool usable in CI (a reference to an unset key is an error, two files defining the same key is an error rather than a silent override), the precedence between the file and the environment already in place — what `--overload` changes when a command runs, and the omission that mirrors it in the printed output — and the observation that a `--env` path that does not exist is silently exit 0. The dialect matrix is exercised as a differential: a `\n` that stays two characters under the POSIX rules and becomes a newline under Ruby, Node and Go, and a key with a space after it that POSIX and Docker refuse while Ruby accepts. All six output formats are pinned exactly, alongside an `export` round trip, and `--sort` is pinned as observed — it reorders without producing sorted order, and is ignored entirely alongside `--name-only`.
- Third-party E2E suite for [getoptions](https://github.com/ko1nksm/getoptions), in `test/e2e/thirdparty/getoptions/`, which tests a generator by running what it generates: every parser the scenarios produce is sourced and driven the way a command line is. The generator half pins the printed example going straight back in as input, the indentation and comment options changing the artifact while both versions still parse, the library with and without its shellcheck directives, and the embed round trip — a script with the parser embedded runs with getoptions nowhere on PATH, and erasing the block returns the original file byte for byte. The generated parsers are then exercised across long and short forms, clustering, `--{no-}` negation and the `+f` spelling, counters, optional arguments falling back to their `on:` default, `--` ending the options, abbreviations accepted while unambiguous and refused with the candidates listed when not, and one distinct message per mistake. Two traps are pinned as observed rather than as documented: a definition generated without the argument it references writes an empty parser at exit 0, and a `validate:` naming a function the script never defines refuses valid input with 127 in the message.
- Third-party E2E suite for [git-secrets](https://github.com/awslabs/git-secrets), in `test/e2e/thirdparty/git-secrets/`, covering the scanner and the hooks that enforce it. No real credential appears anywhere in it: the prohibited patterns are invented by the scenario that uses them, which exercises every rule the tool has. Pinned are where a pattern is stored (the repository's git config), what a match looks like and the fact that all of it — the finding, the error, and the list of mitigations — goes to stderr while stdout stays empty, both ways to allow a false positive, `--literal` escaping metacharacters so they mean themselves, and the exit codes 0, 1, 2 and 129 — including the silent exit 0 that a directory scanned without `-r` comes back with, which is a clean report on a tree that was never read. The hooks are asserted through git itself: the three files installed and their contents, the refusal to overwrite them, a commit blocked with the history as the oracle rather than the error text, a commit refused for its message alone, and `--no-verify` letting a secret into history where `--scan-history` then finds it, quoting the commit. The installer's exit 127 on a `say` it never defines is pinned as observed, and skipped on macOS, where a program of that name exists.
- Third-party E2E suite for [rbenv](https://github.com/rbenv/rbenv), in `test/e2e/thirdparty/rbenv/`, which tests a Ruby version manager without installing a Ruby: a version is a directory under the root, so every scenario builds its own root out of two-line executables and drives the real resolution rules against them. The selection state machine is pinned end to end — the file `global` writes, the `.ruby-version` `local` writes, the environment variable that outranks both, the origin reported for each choice, `--unset` deleting the file rather than blanking it, and the refusals that leave every one of those files untouched, proven with exhaustive `changes:` deltas rather than with the error text. The executable half is asserted the way a user meets it: with the shim directory on PATH, `ruby` answers 3.2.0 or 3.3.1 according to the directory it runs in while PATH never changes. Alongside it, `rehash` adding exactly what is new, `exec` passing arguments through and answering 127 for a command that is not there, a plugin hook running with the version already resolved, the integration `init - bash` prints, and `init` editing exactly one profile inside a sandboxed home and nothing at all on a second run.
- Third-party E2E suite for [ShellSpec](https://shellspec.info/), in `test/e2e/thirdparty/shellspec/`: the BDD test framework for POSIX shells, whose own suite is written in ShellSpec, pinned here from outside. Its three-level exit contract is asserted at every level — 1 for a command line it cannot use, 101 for specs that failed, 102 for a specfile that will not parse — together with both overrides and the two defaults worth knowing: an example that asserts nothing is a failure, and finding no examples at all is not. Selection is checked on both halves (counting and listing without running, a dry run that reports a failing example as ok, tags, focus, line and id ranges, the file pattern, and the surprise that `--example` matches a whole name rather than a substring), and quick mode gets its full state machine: the ledger appearing, the replay narrowing to the failure, the ledger emptying once it passes, the next run going wide again. Every formatter is pinned in its own shape, with `--output` report files, `--profile`, and the `--xtrace` log beside them, and `--random` is recorded as it behaves — the verdict intact, the TAP plan line malformed. What a specfile can do is proven with an append-only log: hook ordering, `Include` with call against run, `Data`, `Parameters`, mock scoping, the `--sandbox` that removes PATH, the runnable `--init` scaffold, and the report colors checked on a real terminal with `screen.attrs`.
...
- The rbenv suite asserts the version list the same way on every machine. Two scenarios read the plain `rbenv versions` output, which carries a `system` entry when the host has a Ruby of its own — true on the CI runner and false on the machine the suite was written on, so the suite passed locally and failed on the runner it was written for. The listing is now asserted with `--bare`, which is exactly the flag that excludes the host's Ruby, and the marker line is matched on its own rather than as part of a whole-output comparison.
- The mommy suite stops depending on two things the machine happens to have. Its `--help` is a man page found the way man finds any page — beside the bin directory the program was run from — so the CI leg now puts mommy's built tree on PATH instead of symlinking one file into GOPATH, where the page could not be found; the scenario also pins the fallback, with `MANPATH` pointed at nothing, where mommy names the README for its own version. And `-p` is documented to work only where `/bin/sh` supports pipefail, which is true on the machine the suite was written on and false on the CI runner, so the two outcomes are now two scenarios gated on that exact condition: the head of the pipeline deciding the verdict, and the shell refusing the option with exit 2 while the message about the command is printed anyway.
- The git-extras changelog scenario names the editor it wants. After writing the file, `git changelog` opens it in `git var GIT_EDITOR` — which is `true` on the machine the suite was written on and nano on a CI runner, where it then owned the terminal until atago killed the session. The pty steps now set `GIT_EDITOR=true`, so the scenario is about the file it wrote rather than about which editor the host happens to ship.
- The git-extras suite drives `git changelog` on a terminal, because that is the only place it works. The scenario ran it through a pipe, which passes on a machine that has a controlling terminal to fall back on and fails on a CI runner that has none — `stty` reports "Standard input is not a terminal" and the command exits 1 without writing the file it exists to write. It now runs in a `pty:` step, like the two reporting scenarios that needed one, and the comment says why. Reproducing this locally needs `setsid … < /dev/null`, which is what a runner actually gives a process.
...
- The GnuPG suite drives the interactive passphrase prompt. With a terminal attached and no loopback pinentry, the agent hands the prompt to pinentry, which draws a dialog and waits for a person — the half of gpg no non-interactive suite reaches. The scenario stops the agent so the passphrase cannot come from its cache, declares a terminal large enough for the dialog to draw, types the passphrase, and proves the decryption landed by comparing the recovered file byte for byte. The passphrase is declared under `secrets:` so a failing transcript cannot print it.
- Third-party E2E suite for [GnuPG](https://gnupg.org/), in `test/e2e/thirdparty/gpg/`, built around round trips and refusals rather than happy paths: encrypt/decrypt returns the original bytes and the ciphertext never carries the plaintext, ASCII armor decodes back byte for byte, a single flipped byte and a keyring without the secret key are refused rather than approximated, a detached signature stops verifying the moment the signed file changes, and symmetric encryption answers only to its own passphrase. The three levels of gpg's exit contract are pinned (0, 1 for a bad signature, 2 for everything else) alongside the `--status-fd` lines integrations read. Every key is generated inside the scenario workdir and key lookup is disabled, so nothing is committed and nothing reaches the network.
- Third-party E2E suite for [kubectx and kubens](https://github.com/ahmetb/kubectx), in `test/e2e/thirdparty/kubectx/`, reproducing all 31 tests of both upstream Bats suites and then pinning what a Bats `run` cannot see: which stream each line goes to (context and namespace names on stdout, status lines on stderr), the kubeconfig on disk as the oracle for every switch, rename, and delete, exhaustive `changes:` deltas showing that a refused name writes nothing, the previous-context and per-context previous-namespace state landing under a sandboxed home instead of the real one, the non-atomic batch delete, `kubens --force`, and the fzf picker driven inside a pseudo-terminal. No cluster is contacted: every kubeconfig is a throwaway fixture in the scenario workdir.
...
- A list or a mapping written where one value belongs names the key and the shape it wants. `matches: ["^a", "b$"]` — the natural mistake next to `contains:`, which does take a list — was refused with the decoder's own message, `cannot unmarshal []interface {} into Go struct field Spec.Scenarios of type string`: true, and useless, since it names a Go type and the outermost field rather than the key one line above the caret. The hint now reads the key off the excerpt the error already prints, says what shape it takes, and for the pattern matchers points at the substring matchers that accept a list.
- A `${...}` reference nothing can resolve fails the same way wherever a process is started from it. Every entry inside a `pty:` session was guarded, while the command that starts the session was not: `command: "myapp ${env:PROFILE}"` with `PROFILE` unset launched a program with the literal text as an argument, so the failure arrived from the program rather than from the mistake in the spec. The pty command and cwd now go through the run step's guard, with the same shell rule — a bare `${name}` under `shell: true` is still the shell's business, an `${env:NAME}` no shell understands is not. The verdict also carries ATG4505 everywhere now; a run step's version of this failure was the one uncoded message in the family, so `atago explain` had nothing to say about it.
- The published JSON Schema accepts an unquoted scalar wherever atago takes text. Every text property was typed `string` alone, so `env: {PORT: 8080}` — the way anyone writes a port — was flagged by a schema-validating editor on a spec atago loads and means literally. The duration properties stay strict, since `timeout: 30` is not a duration in any reading and the loader refuses it, and a walk over the document now fails the build when another text property is typed strictly without that being a decision.
- The published JSON Schema accepts every spec this repository ships. `assert.file` modeled its matcher list with `oneOf`, so the composable shapes the loader documents — a `size` bound beside a content matcher, `min_size` with `max_size` — were editor errors on specs atago runs, and a `store` capture's json node reached the same definition as an assertion's, whose `oneOf` demanded a matcher the capture cannot carry: every store-by-json-path spec failed validation, three shipped examples among them. Fourteen files were affected while the conformance guard stayed green, because it validated two demo specs; it now walks the whole tree, and the loader's own dogfood test reads that same corpus, so a spec only one of them sees no longer exists.
- The file assertion's no-matcher diagnostic names every key that satisfies it. `file: {path: out.txt}` was answered with the eight content matchers, while a size bound alone is a complete file assertion — `size: 0` says the failed run left an empty file rather than a half-written one — so three of the eleven ways out were hidden from the one message that exists to list them. The exclusivity diagnostic still names only the content matchers, which are the keys that actually compete, and a walk over the assertion's fields now fails the build when a matcher is neither named in the message nor recorded as something other than a matcher.
- A plain YAML scalar in a text field means the characters the author wrote. YAML types an unquoted `1.20` as a float and an unquoted `TRUE` as a boolean, and decoding one into a text field kept the parsed value and dropped the text: `contains: 1.20` asserted the substring `1.2` — which matches output that never carried `1.20` and passes a test that should fail — `equals: 1.20` reported an expectation of `1.2` that appears nowhere in the spec, `env: {V: 007}` exported `7` to the program under test, and a fixture written `content: 0x10` wrote `16`. The source text is now restored wherever the model has a text field, so quoting a value no longer changes what it means. Fields that are genuinely typed are untouched: `exit_code: 007` is still the number 7, and a json matcher's `equals` still compares by JSON type, where the string "true" does not equal the boolean true.
- A failure's evidence reveals a whitespace-only payload instead of printing it invisibly. `empty: false` fails on output of spaces and newlines because whitespace counts as empty, and the `Actual` block rendered exactly those bytes — a blank section that looks even more empty than the `(empty)` a truly empty stream gets, hiding the one fact the verdict turned on. A payload that is nothing but whitespace is now quoted, in every matcher's excerpt; visible payloads keep their unquoted, copy-pasteable form.
- A `changes:` failure shows the delta the assertion judged, and names what `ignore:` dropped. The `Actual` line rendered the raw delta, so an ignored path sat inside `created [...]` as evidence while the exhaustive entry list that omits it raised no problem — a claim and an observation that could not both be true, with nothing saying why. The ignored paths now appear in their own `ignored [...]` group, so the evidence and the verdict agree.
- A Ctrl-C that lands while a `pty:` session's `exec:` helper runs reports ATG4103 (run interrupted), not ATG4101 (step timeout). The message already said "canceled", but the code's published fix told the author to raise `timeout:` — advice about a bound that had nothing to do with what happened. The helper's own budget running out keeps reporting the timeout code.
...
- The Windows E2E leg runs the whole self-hosted suite instead of a hand-kept subset, and every target is classified so the subset cannot quietly shrink again. Windows ran 37 of the 82 spec files; the other 45 were left out because their scaffolding is POSIX — `printf`, `cat`, a `for` loop — and nothing recorded that, tested it, or would have noticed a forty-sixth joining them. `scripts/windows_specs.tsv` now classifies every target as `cmd` (runs under the native cmd.exe), `bash` (runs with `ATAGO_SHELL` pointed at the bash that ships with Git for Windows, where the shell is scaffolding and atago's own engine is the subject), or `none` with a written reason, and a unit test fails when a spec is added without a decision. Both Windows CI legs expand the same table, so they cannot drift from each other. Windows went from 238 passing self-hosted scenarios to 550 in the same job; one file is `none`, because a symlink, `fixture.mode` permission bits, and `date -r` have no Windows counterpart at all.
- Six scenarios stopped needing a shell they only used to set a variable or swallow an exit code. `VAR=value atago run …` is `run.env:`, and `atago run …; true` is an `exit_code:` assert — a run step's non-zero exit is only a failure when a spec asserts on it. Both were reaching for shell syntax to get something atago already offers, and both broke the moment the interpolated path had backslashes in it.
- A recursive `dir:` count failure speaks in files, not entries. The recursive counter looks at FILES only, while the listing under Actual shows every walked path — directories and links included — so a hint saying "has 3 entries" beside a four-line listing contradicted both the number a reader can count there and the rule the count applies. The failure is now described as a file count, the hint says how many files the tree holds, and a bound of one reads "exactly 1 file in the tree" instead of "1 files". The direct (non-recursive) count keeps its entry vocabulary, where every directory entry counts and "entries" is the truthful word.
- The console diff keeps its removal red and its addition green for content lines that begin with `--` or `++` followed by a space. A removed line beginning that way — a SQL comment, say — renders with the diff marker in front as `--- select`, which the colorizer read as the file label and dimmed; the added twin went dim instead of green the same way. The label check is positional now, since the two label lines only ever open the diff; hunk headers need no such guard, because every content line carries a marker byte and only a header can start with `@@` and a space.
- An `examples/` spec that runs NOTHING on Windows is recorded as such, and cannot become one silently. The README calls `examples/` the syntax reference and says it is tested on Linux, macOS, and Windows, and the test that runs them accepts a fully skipped spec as green — so fifteen of the thirty-seven hermetic examples were executing nothing there while reporting exactly like the ones that passed. Each is now listed with the reason it cannot run, the list is exact rather than a floor (an example that starts running a scenario on Windows fails the test until its entry is removed), and on POSIX nothing may be dead at all, since an example that runs nothing on the platform it was written for is broken rather than unportable. The `stdin` example came off the list on the way: `sort` reads standard input on Windows as well as POSIX, so the two scenarios that only needed a program to echo their input back are the same demonstration everywhere instead of a POSIX-only one.


### Documentation

- The reference states what differs on Windows, in a table. atago has run on three platforms for a long time and the handful of real differences — `signal:` steps, ConPTY's Windows 10 floor, executability by `PATHEXT` rather than a mode bit, `fixture.mode` and symlinks, the permission component `changes:` leaves out, the `sandbox_home` and `clear_env` variable families, and which shell `shell: true` runs — were each documented where they were implemented and nowhere a reader would look. A section that says so once, with the reason each time, is the difference between a platform being supported and a platform being tested.
- The CI guide shows a three-OS matrix and what makes the Windows leg worth having rather than red: gate the genuinely POSIX scenarios with `skip: {os: windows}` instead of deleting them, prefer `run.env:` / `run.stdin:` / `run.stdout_to:` over a shell so a command is argv rather than syntax, and point `ATAGO_SHELL` at Git for Windows' bash where a POSIX shell really is needed. The page had no Windows content at all.
