...
- `pty.graphics: kitty` makes the pty answer like a terminal that draws images with the kitty graphics protocol, and `screen.images` asserts the images a program drew. A program that shows pictures inline asks the terminal first, and one that requires them exits when the answer is no, so on the plain pty it could only ever be tested failing. With the key set, the capability query is acknowledged, the cell-size query (`CSI 16 t`) reports 10x20-pixel cells, and every directly transmitted image (PNG, raw RGB or RGBA, chunked or zlib-compressed) is decoded and kept. `screen.images` takes `count` or `min_count`, and `contains` entries that borrow the `image:` assertion's constraints: size bounds, `alpha`, and `similar_to` with `max_diff`. An entry holds when some drawn image meets all of its constraints, since a program that downloads images concurrently draws them in no fixed order. `expect_screen:` takes the same `images:` to wait for an image to arrive, except `similar_to`, whose baseline needs the spec directory a mid-session wait does not have. On a failed `contains`, every drawn image is written to `--artifacts-dir`.
