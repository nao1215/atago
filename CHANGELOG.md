...
- The console diff keeps its removal red and its addition green for content lines starting with `-- ` or `++ `. A removed line beginning `-- ` — a SQL comment, say — renders as `--- ` with the diff marker in front, which the colorizer read as the file label and dimmed; the added twin went dim instead of green the same way. The label check is positional now, since the two label lines only ever open the diff; hunk headers need no such guard, because every content line carries a marker byte and only a header can start with `@@ `.
