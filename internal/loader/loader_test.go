...
func TestBugHunt_Rejections(t *testing.T) {
	...
	tests := []struct{ name, src, want string }{
		...
		{"pty graphics value", specSteps("pty: {command: sh, graphics: sixel}"), `graphics "sixel" is not supported (the only value is "kitty")`},
		{"screen images empty", specSteps("assert: {screen: {images: {}}}"), "images needs at least one of count/min_count/contains"},
		{"screen images negative count", specSteps("assert: {screen: {images: {count: -1}}}"), "images.count must be >= 0"},
		{"screen images count and min_count", specSteps("assert: {screen: {images: {count: 1, min_count: 1}}}"), "set count or min_count, not both"},
		{"screen images entry needs a constraint", specSteps("assert: {screen: {images: {contains: [{}]}}}"), "contains[0] must set at least one of width/height"},
		{"screen images entry bounds", specSteps("assert: {screen: {images: {contains: [{min_width: 9, max_width: 3}]}}}"), "min_width 9 exceeds max_width 3"},
		{"screen images max_diff needs similar_to", specSteps("assert: {screen: {images: {contains: [{width: 1, max_diff: 0.1}]}}}"), "max_diff requires similar_to"},
		{"expect_screen images no similar_to", specSteps("pty: {command: sh, session: [{expect_screen: {images: {contains: [{similar_to: a.png}]}}}]}"), "similar_to is not supported in expect_screen"},
