...
func TestBugHunt_Rejections(t *testing.T) {
	...
	tests := []struct{ name, src, want string }{
		...
		{"pty expect and send", specSteps("pty: {command: sh, session: [{expect: hi, send: x}]}"), "set exactly one of expect/send/expect_screen/resize (got more than one)"},
		{"pty neither expect nor send", specSteps("pty: {command: sh, session: [{}]}"), "set exactly one of expect/send/expect_screen/resize"},
		...
		{"pty send key and paste", specSteps("pty: {command: sh, session: [{send: {key: left, paste: hi}}]}"), "set either {key: <name>} or {paste: <text>}"},
		...
		{"pty resize with send", specSteps("pty: {command: sh, session: [{resize: {rows: 4, cols: 8}, send: x}]}"), "set exactly one of expect/send/expect_screen/resize (got more than one)"},
