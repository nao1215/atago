package loader

import (
	"fmt"
	"reflect"
	"slices"
	"strconv"
	"strings"
	"sync"

	"github.com/nao1215/atago/internal/spec"
	"github.com/nao1215/atago/internal/yaml"
)

// suggestUnknownField appends a "did you mean" hint to an unknown-field parse
// error when the misspelled name is close to a real spec field. A typo like
// `asserts:` or `stdut:` is the most common first-five-minutes mistake, and
// the raw error alone makes the user diff the docs by eye.
func suggestUnknownField(msg, key string) string {
	// The name is a real spec field, just not valid here: almost always an
	// indentation slip (e.g. `command:` dedented out of its `run:` block).
	if isKnownField(key) {
		return fmt.Sprintf("%s\nhint: %q is a spec field, but not at this position — check the indentation and nesting", msg, key)
	}
	best, ok := closestField(key)
	if !ok {
		return msg
	}
	return fmt.Sprintf("%s\nhint: did you mean %q?", msg, best)
}

// streamTargets are the assertion targets that take a matcher mapping.
var streamTargets = []string{"stdout", "stderr", "body", "rows", "message", "value"}

// suggestShape appends a hint for a value written in a shape its key cannot
// take, naming the key the author wrote rather than the path to it.
func suggestShape(msg string, e *yaml.Error) string {
	key := lastKey(e.Path)
	if key == "" {
		return msg
	}
	// A stream assertion written as a bare scalar — `stdout: hello` instead
	// of `stdout: {contains: hello}` — is the single most common first-spec
	// mistake, and "expected a mapping" gives no clue what shape is wanted.
	if e.Want == "a mapping" && e.Got != yaml.MappingNode && slices.Contains(streamTargets, key) {
		return fmt.Sprintf("%s\nhint: %s must set a matcher mapping, e.g. %s: {contains: \"...\"} or %s: {equals: \"...\"}", msg, key, key, key)
	}
	if e.Got != yaml.SequenceNode && e.Got != yaml.MappingNode {
		return msg
	}
	written := "a list"
	if e.Got == yaml.MappingNode {
		written = "a mapping"
	}
	var hint string
	switch e.Want {
	case "a string":
		hint = fmt.Sprintf("%q takes a single value, not %s", key, written)
	case "a number":
		hint = fmt.Sprintf("%q takes a single number, not %s", key, written)
	case "true or false":
		hint = fmt.Sprintf("%q takes true or false, not %s", key, written)
	default:
		return msg
	}
	// A list of patterns is the common shape of this mistake, and the substring
	// matchers next to it are the ones that do take a list — so say which is
	// which rather than leaving the author to guess that one key differs.
	if written == "a list" && (key == "matches" || key == "not_matches") {
		hint += "; write one assert per pattern (the substring matchers \"contains\" and \"not_contains\" are the ones that take a list)"
	}
	return msg + "\nhint: " + hint
}

// lastKey returns the last mapping key of an error path,
// "scenarios[0].steps[1].run.command" -> "command", or "" when the path ends in
// a list index or names no key.
func lastKey(path string) string {
	if path == "" || strings.HasSuffix(path, "]") {
		return ""
	}
	if strings.HasSuffix(path, `"`) {
		// A quoted key may hold dots of its own.
		if i := strings.LastIndex(path[:len(path)-1], `"`); i >= 0 {
			path = path[i:]
		}
	} else if i := strings.LastIndexByte(path, '.'); i >= 0 {
		path = path[i+1:]
	}
	if unq, err := strconv.Unquote(path); err == nil {
		return unq
	}
	return path
}

// isKnownField reports whether name is a field somewhere in the spec model.
func isKnownField(name string) bool {
	return slices.Contains(fieldVocabulary(), strings.ToLower(name))
}

// closestField returns the spec field name nearest to the typo, if any is
// within an edit distance small enough to be a plausible slip.
func closestField(typo string) (string, bool) {
	lower := strings.ToLower(typo)
	best, bestDist := "", 3 // allow up to 2 edits; anything further is guessing
	for _, name := range fieldVocabulary() {
		if name == lower {
			continue // same name: the field exists elsewhere; a distance-0 hint would mislead
		}
		if d := editDistance(lower, name); d < bestDist {
			best, bestDist = name, d
		}
	}
	return best, best != ""
}

var (
	vocabOnce sync.Once
	vocab     []string
)

// fieldVocabulary collects every yaml field name reachable from spec.Spec by
// walking the struct tags reflectively, so the hint list can never drift from
// the model.
func fieldVocabulary() []string {
	vocabOnce.Do(func() {
		seen := map[string]bool{}
		visited := map[reflect.Type]bool{}
		var walk func(t reflect.Type)
		walk = func(t reflect.Type) {
			for t.Kind() == reflect.Pointer || t.Kind() == reflect.Slice || t.Kind() == reflect.Array || t.Kind() == reflect.Map {
				t = t.Elem()
			}
			if t.Kind() != reflect.Struct || visited[t] {
				return
			}
			visited[t] = true
			for f := range t.Fields() {
				tag, _, _ := strings.Cut(f.Tag.Get("yaml"), ",")
				if tag != "" && tag != "-" {
					seen[tag] = true
				}
				walk(f.Type)
			}
		}
		walk(reflect.TypeFor[spec.Spec]())
		for name := range seen {
			vocab = append(vocab, name)
		}
		// Sorted, so a typo equally close to several fields gets the same
		// hint in every process instead of whichever map order came first.
		slices.Sort(vocab)
	})
	return vocab
}

// editDistance is the Levenshtein distance between two short field names.
func editDistance(a, b string) int {
	if a == b {
		return 0
	}
	la, lb := len(a), len(b)
	prev := make([]int, lb+1)
	cur := make([]int, lb+1)
	for j := 0; j <= lb; j++ {
		prev[j] = j
	}
	for i := 1; i <= la; i++ {
		cur[0] = i
		for j := 1; j <= lb; j++ {
			cost := 1
			if a[i-1] == b[j-1] {
				cost = 0
			}
			cur[j] = min(prev[j]+1, cur[j-1]+1, prev[j-1]+cost)
		}
		prev, cur = cur, prev
	}
	return prev[lb]
}
