...
type PTY struct {
	...
	// program transmits directly (t=d) is recorded for `screen.images` until the program deletes it by id, number, id range, or all (a delete by screen position is not followed). Unset, the terminal
