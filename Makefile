...
thirdparty: ## Run atago against third-party programs (needs git, caddy, helix, pushgateway, webhook, restic, rclone, minio+mc, prometheus+promtool, gitea, coredns+dig, nats-server+nats, mailpit, ntfy, yazi, gum on PATH)
	env CGO_ENABLED=0 $(GO_BUILD) $(GO_LDFLAGS) -o ./dist/$(APP) .
